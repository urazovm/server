...
exports.domainL = function(er) 
{
	...
	for(var key in GLOBAL.errorsLists.serverErrorsList)
	{
		if(GLOBAL.errorsLists.serverErrorsList[key].error === er.stack)
		{
			if(GLOBAL.errorsLists.serverErrorsList[key].state == 1)
			{
				...
				SQL.queryAsync("UPDATE `game_ErrorsServerList` SET `state` = 2 WHERE `id` = "+key);
			}
			find_error = true;
			errorId = key;
			break;
	...
	SQL.queryAsync("INSERT INTO `game_ErrorsServer` SET `date` = UNIX_TIMESTAMP(), `errorId` = "+errorId );
